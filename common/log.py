...
LOG_PATH = LOG_BASE_PATH + now_time + "/"
try:
...
class LoggerProject(object):
    ...
    def _file_logger(self):
        time_rotate_file = TimedRotatingFileHandler(filename=LOG_PATH + self.name, when='D', interval=1,
                                                    backupCount=30)
...
log_api = LoggerProject('mcp-chatbi.log')
